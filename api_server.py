...
async def status(uid: str):
    ...
    This endpoint allows tracking the progress of a previously submitted task.
    When the task is complete, it returns the generated model as base64 encoded data.
    
    Args:
        uid: The unique identifier returned by the /send endpoint
        
    Returns:
        StatusResponse: The current status of the task and model data if completed
    """
    initial_file_path = os.path.join(SAVE_DIR, f"{uid}.glb")
    textured_file_path = os.path.join(SAVE_DIR, f"{uid}_textured.glb")
    
    # Check if textured file exists (generation completed with textures)
    if os.path.exists(textured_file_path):
        try:
            with open(textured_file_path, 'rb') as f:
                model_data = f.read()
                model_base64 = base64.b64encode(model_data).decode('utf-8')
            response = {'status': 'completed', 'model_base64': model_base64}
