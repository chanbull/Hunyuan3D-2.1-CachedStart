...
def generation_all(
    ...
    # テクスチャ生成にもタイムアウトを追加
    print(f"Starting texture generation for mesh: {path}")
    ...
    def timeout_handler():
        ...
        if not timeout_flag.is_set():
            print("ERROR: Texture generation timed out after 10 minutes")
    ...
    try:
        ...
        timeout_flag.set()  # タイムアウトをクリア
        print(f"Texture generation completed successfully: {path_textured}")
    except Exception as e:
        timeout_flag.set()
...
Fast for very complex cases, Standard seldom use.',
        ...
        def on_gen_textured_shape(*inputs):
            if not HAS_TEXTUREGEN:
                raise gr.Error('Texture generation is not available. Please check your environment and requirements.')
            return generation_all(*inputs)

        btn_all.click(
            on_gen_textured_shape,
            inputs=[
                caption,
                image,
                mv_image_front,
...
if __name__ == '__main__':
    ...
    def get_free_port(default_port):
        ...
        try:
            ...
            s.listen(1)
            port = s.getsockname()[1]
            print(f"[INFO] Port {default_port} is in use. Using free port {port} instead.")
        finally:
            s.close()
        return port

    args.port = get_free_port(args.port)

    SAVE_DIR = args.cache_path
    os.makedirs(SAVE_DIR, exist_ok=True)

    # Setup model cache directories for persistence
    MODEL_CACHE_BASE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "model_cache")
    HY3DGEN_CACHE = os.path.join(MODEL_CACHE_BASE, "hy3dgen")
    HF_CACHE = os.path.join(MODEL_CACHE_BASE, "huggingface")
    U2NET_CACHE = os.path.join(MODEL_CACHE_BASE, ".u2net")

    # Create cache directories
    os.makedirs(HY3DGEN_CACHE, exist_ok=True)
    os.makedirs(HF_CACHE, exist_ok=True)
    os.makedirs(U2NET_CACHE, exist_ok=True)
    os.makedirs(os.path.join(HF_CACHE, "hub"), exist_ok=True)

    # Set environment variables for model caching
    os.environ["HY3DGEN_MODELS"] = HY3DGEN_CACHE
    os.environ["HF_HOME"] = HF_CACHE
    os.environ["TRANSFORMERS_CACHE"] = HF_CACHE
    os.environ["HF_HUB_CACHE"] = os.path.join(HF_CACHE, "hub")
    os.environ["PYTORCH_TRANSFORMERS_CACHE"] = os.path.join(HF_CACHE, "pytorch_transformers")
    os.environ["HF_DATASETS_CACHE"] = os.path.join(HF_CACHE, "datasets")

    print(f"Model cache setup:")
    print(f"  HY3DGEN_MODELS: {HY3DGEN_CACHE}")
    print(f"  HF_HOME: {HF_CACHE}")
    print(f"  U2NET cache: {U2NET_CACHE}")
    print(f"  Gradio WebUI will launch on port: {args.port}")
    ...
    if not args.disable_tex:
        try:
            ...
            # Auto-download RealESRGAN_x4plus.pth if not exists
            realesrgan_path = "hy3dpaint/ckpt/RealESRGAN_x4plus.pth"
            if not os.path.exists(realesrgan_path):
                print("RealESRGAN_x4plus.pth not found, downloading...")
                ...
                try:
                    ...
                    print(f"Successfully downloaded RealESRGAN_x4plus.pth to {realesrgan_path}")
                except Exception as download_error:
                    print(f"Failed to download RealESRGAN_x4plus.pth: {download_error}")
                    print("Please manually download from: https://github.com/xinntao/Real-ESRGAN/releases/download/v0.1.0/RealESRGAN_x4plus.pth")
            ...
            print("Creating texture generation config...")
            # より軽量な設定でテクスチャ生成を試行
            conf = Hunyuan3DPaintConfig(max_num_view=4, resolution=512)  # 軽量化：view数を8→4、解像度768→512に
            conf.realesrgan_ckpt_path = "hy3dpaint/ckpt/RealESRGAN_x4plus.pth"
            conf.multiview_cfg_path = "hy3dpaint/cfgs/hunyuan-paint-pbr.yaml"
            conf.custom_pipeline = "hy3dpaint/hunyuanpaintpbr"
            print("Initializing texture generation pipeline (this may take several minutes on first run)...")
            print("Loading texture generation models - please wait...")
            print("Note: If this takes longer than 10 minutes, there may be an issue.")
            print("Using optimized settings: 4 views, 512 resolution for faster processing")
            import time
            import threading
            ...
            def timeout_handler():
                ...
                if not timeout_flag.is_set():
                    print("ERROR: Texture pipeline initialization timed out after 10 minutes")
            
            start_tex_time = time.time()
            timeout_thread = threading.Thread(target=timeout_handler, daemon=True)
            timeout_thread.start()
            
            try:
                print("Step 1/3: Creating Hunyuan3DPaintPipeline instance...")
                tex_pipeline = Hunyuan3DPaintPipeline(conf)
                timeout_flag.set()  # タイムアウトをクリア
                tex_init_time = time.time() - start_tex_time
                print(f"Texture generation pipeline initialized successfully in {tex_init_time:.2f} seconds")
            except Exception as e:
                timeout_flag.set()
                print(f"ERROR: {e}")
                print("Texture generation initialization failed due to timeout.")
                raise Exception("Texture pipeline initialization timeout - please check system resources and dependencies")
            except Exception as e:
                timeout_flag.set()
                tex_init_time = time.time() - start_tex_time
                print(f"ERROR during texture pipeline initialization after {tex_init_time:.2f} seconds: {e}")
                raise
            ...
            HAS_TEXTUREGEN = False
            # テクスチャ生成無効時のダミー関数を定義
            def dummy_tex_pipeline(*args, **kwargs):
                raise RuntimeError("Texture generation is not available due to initialization failure")
            tex_pipeline = dummy_tex_pipeline
    ...
    def open_browser_when_ready(url, timeout=30):
        ...
        for _ in range(timeout * 10):
            try:
                import socket
                s = socket.create_connection((args.host if args.host != '0.0.0.0' else '127.0.0.1', args.port), timeout=0.5)
                s.close()
                break
            except Exception:
                time.sleep(0.1)
    ...
    # devcontainerやLinuxでもローカルアクセス用URLを生成
    browser_url = f"http://127.0.0.1:{args.port}/"
    threading.Thread(target=open_browser_when_ready, args=(browser_url,), daemon=True).start()
    
    # uvicorn起動時にポート競合があれば再度自動選択
    max_retries = 5
    for attempt in range(max_retries):
        try:
            ...
            if "address already in use" in str(e).lower() and attempt < max_retries - 1:
                print(f"[WARN] Port {args.port} is in use, finding another port...")
                args.port = get_free_port(args.port + 1)
                print(f"[INFO] Retrying with port {args.port}")
                # ブラウザURLも更新
                browser_url = f"http://127.0.0.1:{args.port}/"
            else:
                raise
